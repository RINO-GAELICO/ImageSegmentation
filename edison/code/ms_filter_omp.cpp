...
void msImageProcessor::Filter_omp(float sigmaS, float sigmaR)
{
...
#pragma omp for private(j, k) schedule(dynamic)
		for (i = 0; i < L; i++)
		{
			...
			// Allcocate memory for yk inside the Thread part
			double *yk = new double[lN];

			// Allocate memory for Mh inside the Thread part
			...
			for (j = 0; j < 27; j++)
			{
				...
				while (idxd >= 0) // if it is not -1 it means that there is more points in the bucket
				{
					idxs = lN * idxd;
					...
					{
						el = sdata[idxs + 2] - yk[2]; // store the intensity of the pixel
						if (yk[2] > hiLTr)
							diff = 4 * el * el;
						else
							diff = el * el;
						...
						if (diff < 1.0)
						{
							...
							for (k = 0; k < lN; k++)
								Mh[k] += weight * sdata[idxs + k]; // we add the weighted value of the pixel to the mean shift vector
							wsuml += weight;
						}
					}
					idxd = slist[idxd];
			...
			// calcualte the value of the pixel
			float pixValue = (float)(yk[2] * sigmaR);
			#pragma omp critical
			{
				// store result into msRawData...
				msRawData[i] = pixValue;
			}
