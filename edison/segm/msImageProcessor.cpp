...
void msImageProcessor::Filter(int sigmaS, float sigmaR, Implementation implementation)
{
	...
	// Serial code
	case SERIAL:
		FilterSerial((float)(sigmaS), sigmaR);
		break;
	// omp code
	case OPEN_MP:
		Filter_omp((float)(sigmaS), sigmaR);
		break;
	case CUDA:
		Filter_cuda((float)(sigmaS), sigmaR);
		break;
	}
...
void msImageProcessor::Segment(int sigmaS, float sigmaR, int minRegion, Implementation implementation)
{
	...
	Filter(sigmaS, sigmaR, implementation);
